...
/*
思路: 双指针
用一个指针 slow 表示当前可以放置的元素位置，另一个指针 fast 遍历数组。
对于数组中每个元素，检查是否满足出现次数不超过两次的条件：
如果 fast 指针所指的元素与 slow - 2 指针的元素不同（说明没有超出两次），将该元素放到 slow 指针所指的位置。
每次满足条件后，移动 slow 指针。
最终 slow 指针的值就是数组的新长度。
*/
