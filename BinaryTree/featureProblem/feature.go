...
第二种情况(notInvited):他没被邀请参加，那么递归终止条件是，如果他没有下属，那么直接返回的快乐值0。
如果他有下属，那么他的下属可以参加，也可以不参加(因为员工的快乐值是不定的，可能很大，也可能很小，譬如是负数)
参不参加，取决于参加得到的快乐值是否超过不参加得到的快乐值，所以应该遍历他的所有直接下属，累加该下属的所能
提供的最大快乐值(res +=Max(isInvited(sub), notInvited(sub)),遍历完后返回

最后就是从根节点出发，分别得出员工被邀请参加派对，或者不被邀请所得到的快乐值，取较大值即可。
*/
