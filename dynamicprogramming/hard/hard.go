...
/*
思路: 动态规划
本题可以使用动态规划来解决。我们创建一个二维布尔数组 dp，其中 dp[i][j] 表示字符串 s 的前 i 个字符和模式 p 的前 j 个字符
是否匹配。

状态转移方程如下：
当 p[j-1] 是普通字符或 '?' 时：
如果 s[i-1] == p[j-1] 或 p[j-1] == '?'，则 dp[i][j] = dp[i-1][j-1]。

当 p[j-1] 是 '*' 时：
dp[i][j] = dp[i-1][j] || dp[i][j-1]。
dp[i-1][j] 表示 '*' 匹配了 s 的一个或多个字符。
dp[i][j-1] 表示 '*' 匹配了空字符序列。

初始化：
dp[0][0] = true，空字符串和空模式是匹配的。
当 s 为空时，如果 p 的前 j 个字符都是 '*'，则 dp[0][j] = dp[0][j-1]。
*/

func isMatchTwo(s string, p string) bool {
	m, n := len(s), len(p)
	// 创建 dp 数组
	dp := make([][]bool, m+1)
	for i := 0; i <= m; i++ {
