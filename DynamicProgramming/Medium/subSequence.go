...
1 确定dp数组以及下标的含义
布尔类型的dp[i][j]：表示区间范围[i,j]（注意是左闭右闭）的子串s[i:j+1]是否是回文子串，如果是, dp[i][j]为true，否则为false。
...
1 确定dp数组以及下标的含义
这道题目我们要维护两个动规数组。
dp[i]表示i之前(包括i)的最长递增子序列的长度为dp[i]
...
更长的递增子序列。
那么以j为结尾的子串的最长递增子序列的个数，就是最新的以i为结尾的子串的最长递增子序列的个数，即：
count[i] = count[j]。
