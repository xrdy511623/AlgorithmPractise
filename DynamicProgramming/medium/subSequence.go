...
/*
思路:贪心+二分查找
考虑一个简单的贪心，如果我们要使上升子序列尽可能地长，则我们需要让序列上升得尽可能慢，因此我们希望每次在上升
子序列最后加上的那个数尽可能地小。
...
这么分析完之后，可以发现本题说是求绘制的最大连线数，其实就是求两个字数组的最长公共子序列的长度！
...
次数最少。因此，可以计算两个字符串的最长公共子序列的长度，然后分别计算两个字符串的长度和与最长公共子序列的长度之差，
即为两个字符串分别需要删除的字符数，两个字符串各自需要删除的字符数之和即为最少地删除操作的总次数。
*/
...
更长的递增子序列。
那么以nums[i]为结尾的子数组的最长递增子序列的个数，就是最新的以j为结尾的子串的最长递增子序列的个数，即：
count[i] = count[j]。
在nums[i] > nums[j]前提下，如果在[0, i-1]的范围内，找到了j，使得dp[j] + 1 == dp[i]，说明找到了两个相同
长度的递增子序列。
那么以nums[i]为结尾的子串的最长递增子序列的个数，就应该加上以nums[j]为结尾的最长递增子序列的个数，即：
count[i] += count[j];
...
思路: 动态规划
定义状态：
用 dp[i] 表示前 i 个字符可以解码的总数。
状态转移方程：
单字符解码：如果当前字符 s[i-1] 在 '1' 到 '9' 之间，那么可以单独解码为一个字母，对应解码数为 dp[i-1]。
双字符解码：如果当前字符和前一个字符组合成的数在 10 到 26 之间，那么可以解码为一个字母，对应解码数为 dp[i-2]。
综合考虑：dp[i] = dp[i-1] + dp[i-2]，条件是满足单字符解码或双字符解码。

边界条件：
dp[0] = 1 表示空字符串有一种解码方式。
如果第一个字符为 '0'，则无法解码，直接返回 0。
优化空间：
我们只需记录前两个状态，因此可以用两个变量代替数组 dp，进一步优化空间复杂度。
*/

func numDecoding(s string) int {
	// 边界条件，如果字符串s为空或者含有前导零，返回0(无法解码)
	if len(s) == 0 || s[0] == '0' {
		return 0
	}
	// prev2: dp[i-2], prev1: dp[i-1]，不含前导零的话第一个字符解码总数为1
	prev2, prev1 := 1, 1
	// 从第二个字符开始迭代
